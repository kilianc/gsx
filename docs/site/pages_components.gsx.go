...
func ComponentsPage() Page {
	return Page{
		...
		Body: Group{
			Section(
				"basics",
				"Uppercase tags call functions",
				P(
					Text("A component is an ordinary function returning "),
					Code("Node"),
					Text(". An uppercase tag calls it."),
				),
				Split(
					GSX(
						`
...
`,
				...
				P(
					Text("Attributes and children both become "),
					Code("...Node"),
					Text(" arguments, the same shape gomponents' own helpers use."),
				),
				Note(
					P(
						Text("The rule is purely lexical: lowercase tags ("),
						Code("<div>"),
						Text(") are always HTML elements, uppercase tags ("),
						Code("<Card>"),
			...
			Section(
				...
				P(
					Text(
						"A component can take typed parameters instead of only children. Attributes map to parameters ",
					),
					Em("by name"),
					Text(":"),
				),
...
`,
				...
				html.H3(Text("Mixing props and children")),
				P(
					Text("A trailing "),
					Code("...Node"),
					Text(" parameter receives the children; named parameters come from attributes:"),
				),
				Split(
					GSX(
						`
...
`,
				...
				P(
					Text(
						"Because props are function parameters, a typo is a Go compile error and your editor completes them from the signature.",
					),
				),
			),
			Section(
...
`,
				...
				P(
					Text(
						"GSX reads the signatures of functions in your module, so typed props work across packages too.",
					),
				),
			),
			Section(
				...
				P(
					Text("Build a "),
					Code("[]Node"),
					Text(" with an ordinary loop and splice it:"),
				),
				GSX(
					`
var rows []Node
...
`,
				...
				html.H3(Text("Conditionals")),
				P(
					Text("Use "),
					Code("If"),
					Text(" from gomponents for an inline branch, or plain Go for anything larger:"),
				),
				GSX(
					`
return (
...
`,
				...
				html.H3(Text("Raw HTML")),
				P(
					Text("Text is escaped. To emit pre-rendered markup, use gomponents' "),
					Code("Raw"),
					Text(":"),
				),
				...
				Note(
					P(
						Strong("Raw does not escape. "),
						Text(
							"Only pass it markup you generated or have sanitised — never untrusted input.",
						),
					),
				),
			),
